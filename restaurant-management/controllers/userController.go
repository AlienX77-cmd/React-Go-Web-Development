...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// you check if the email already been used by another user
		...
		if count > 0 {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "this email or phone number has already been used"})
			return
		}

		// create some extra details for the user object - get some extra details - created_at, updated_at, ID
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Verify password - compare the password with the hashed password in the database
		...
		// if all goes well, then you generate tokens
