...
func GetOrderItems() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// mongodb to find all order items (empty query) and keep it in result variable
...
func GetOrderItemsByOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while fetching the order items by order ID"})
			return
		}
...
func UpdateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		orderItem.Updated_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
		updateObj = append(updateObj, bson.E{"$set", orderItem.Updated_at})
